...
<?php
$sql_update_danhmuc = "SELECT * FROM danhmuc WHERE id_danhmuc = '$_GET[iddanhmuc]' ";
$query_update_danhmuc = mysqli_query($mysqli, $sql_update_danhmuc);
// echo $query_update_danhmuc;
?>

<h2>Update danh muc san pham</h2>
<form action="modules/quanlydanhmucsp/xuly.php?iddanhmuc=<?php echo $_GET['iddanhmuc'] ?>" method="POST">
  <table border="1">
    <?php
    while ($line = mysqli_fetch_array($query_update_danhmuc)) {
      ?>
      <tr>
        <td>Ten danh muc</td>
        <td><input type="text" name="tendanhmuc" value="<?php echo $line['tendanhmuc'] ?>"></td>
      </tr>
      <tr>
        <td>Thu tu</td>
        <td><input type="number" name="thutu" min="1" max="50" value="<?php echo $line['thutu'] ?>"></td>
      </tr>
      <tr>
